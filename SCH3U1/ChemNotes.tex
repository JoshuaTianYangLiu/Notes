...
\section{Post-Test Notes}
In terms of IE, anion(-) < neutral < cation(+)

Review on the melting points of ionic, polar and non-polar molecules (Which has a higher melting point compared to which)

Properties of ionic, polar, non-polar molecules.
...
Take two or more elements to form a new substance (AKA combination, formation reactions)

\ce{A + B -> AB}

\medskip
\noindent There are three types simple synthesis reactions

\begin{itemize}
    \item Metal/Non-metal reacts with oxygen to make an \textbf{oxide}
    \item Metal and non-metal combine to form a \textbf{binary ionic compound}
    \item Two non-metals for form a \textbf{binary molecule}
\end{itemize}
...
\begin{itemize}
    \item \ce{2Fe_{(s)} + 3O2_{(g)} -> 2Fe2O3_{(s)}}
    \item \ce{2H2_{(g)} + O2_{(g)} -> 2H2O_{(g)}} % TODO: Add the rest
    \item \ce{2K_{(s)} + Cl2_{(g)} -> 2KCl_{(s)}}
    \item \ce{Mg_{(s)} + F2_{(s)} -> MgF2_{(s)}}
    \item \ce{2N2_{(g)} + O2_{(g)} -> 2N2O_{(g)}}
\end{itemize}

\noindent There are still reactions where there's one or more compounds in the reactant

\begin{itemize}
    \item When a non-metallic oxide reacts with water, the product is an \textbf{acid}
    \item A metallic oxide reacts with water the product is a metallic hydroxide, this is a \textbf{base}
\end{itemize}
...
\begin{itemize}
    \item \ce{SO3_{(g)} + H2O_{(l)} -> H2SO4_{(aq)}} % TODO: Add the rest
    \item \ce{CO2_{(g)} + H2O_{(l)} -> H2CO3_{(aq)}}
    \item \ce{CaO_{(s)} + H2O_{(l)} -> Ca(OH)2_{(aq)}}
    \item \ce{Na2O_{(s)} + H2O_{(l)} -> 2NaOH_{(aq)}}
\end{itemize}

There are some examples there synthesis is ambiguous.

\ce{C_{(s)} + O2_{(g)} -> CO2_{(g)}}

\ce{2C_{(s)} + O2_{(g)} -> 2CO_{(g)}}

In this case, don't worry about which is correct. Your teacher should mark them both correct.

\subsection{Decomposition Reactions}

Where a compound breaks apart into elements/other compounds (Opposite of synthesis)

\ce{AB -> A + B}

\begin{itemize}
    \item \textbf{Binary Compound} breaks down into metal and non-metal
    \item \textbf{Compound with a polyatomic ion} breaks down into simple compound and element
    \item A \textbf{Compound with a polyatomic ion} can also break down into two compounds
\end{itemize}
...
\begin{itemize}
    \item \ce{2H2O_{(l)} -> 2H2_{(g)} + O2_{(g)}} % TODO: Add the rest
\end{itemize}

\emph{Note:} Polyatomic ions with a halogen \ce{F, Cl, Br, I, At} will follow the second rule. A quick way is the oxygen (which is in all of those halogen polyatomics) will be where it gets cut off

\ce{KBrO3 -> KBr + O2}

Everything else will follow the third rule.

\subsection{Combustion Reactions}

Reaction with compound and oxygen to form an oxide for both compounds and releases energy (don't quote me on this)

\medskip
\noindent Combustion reactions often go under synthesis, Metals combine with oxygen. 

\noindent It is only combustion only if it releases a ton of energy

\ce{2Mg_{(s)} + O2_{(g)} -> 2MgO_{(s)}} Yes

\ce{2Fe_{(s)} + O2_{(g)} -> 2FeO_{s}} No

Combustion also sometimes involves fuel, made up of carbon and oxygen (hydrocarbons)

These form carbon dioxide and water. % TODO: Add the rest

\medskip
\noindent Without enough oxygen, compounds with carbon has incomplete combustion which leads to carbon monoxide and water.

\subsection{Single Displacedment}

Aka replacement reactions

Where it displaces a element or compound is displaced by different element.

\medskip
\noindent Two types

\begin{itemize}
    \item Metal replacing metal cation \ce{A + BC -> AC + B}
    \item Non-metal (usually halogen) replacing an anion (-) in a compound \ce{DE + F -> DF + E}
\end{itemize}

Eg:
\begin{itemize}
    \item One metal replaces another metal \\\ce{Cu_{(s)} + 2AgNO3_{(aq)} -> Cu(NO3)2_{(aq)} 2Ag_{(s)}}
    \item Hydrogen is treated as a metal \\\ce{Mg_{(s)} + 2HCl_{(aq)} -> MgCl2_{(aq)} + H2_{g}}
    \item Water is treated as an ionic compound \\\ce{2Na_{(s)} + 2H2O_{(l)} -> 2NaOH_{(aq)} + H2_{(g)}}
\end{itemize}


When analyzing single displacement, 
\begin{itemize}
    \item Treat hydrogen as a metal
    \item Treat acids (\ce{HCl}) as ionic compounds in the form of \ce{H+Cl-}
    \item Treat water as ionic, \ce{H+(OH)-}
\end{itemize}

A reactive metal will displace or replace any metal in a compound that is below it in the activity series (Bottom right of periodic table)
...
\medskip
\noindent The same idea follow for non-metals (especially halogens)

\ce{Cl2_{(g)} + 2KBr_{(aq)} -> 2KCl_{(aq)} + Br2_{(l)}}

For halogens

$$F > Cl > Br > I$$

\subsection{Double displacement}

Double displacement involves the exchange of cations between two ionic compounds usually in aqueous solution

\ce{AB + CD -> CB + AD}

Example:

\ce{NaCl_{(aq)} + AgNO3_{(aq)} -> AgCl_{(s)} + NaNO3_{(aq)}}

Not all ionic compounds will undergo double displacement

It is double displacement when:
\begin{itemize}
    \item A solid forms
    \item A gas is produced
    \item A molecular compound is formed
\end{itemize}

Many double displacement involves the formation of a precipitate

\ce{Na2CO3_{(aq)} + 2HCl_{(aq)} -> 2NaCl_{(aq)} + H2CO3_{(aq)}}

BUT

\ce{H2CO3_{(aq)} -> H2O_{(l)} + CO2_{(g)}}

because carbonic acid is unstable.

Would really be

\noindent\ce{Na2CO3_{(aq)} + 2HCl_{(aq)} -> 2NaCl_{(aq)} + H2O_{(l)} + CO2_{(g)}}

\noindent\ce{NH4Cl_{(aq)} + NaOH_{(aq)} -> NaCl_{(aq)} + NH4OH_{(aq)} -> NaCl_{(aq)} + NH3_{(g)} + H2O_{(g)}}

\medskip

\noindent Neutralization reactions are a type of double displacement which produced water.

These involve the reaction of an acid with a base to form water and an ionic compound
